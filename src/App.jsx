...
const tamanhoData = [
  { faixa: "Ate 50",  Amanda: 6, Isabela: 6, Fernanda: 5, Michelle: 8 },
...
function SectionTitle({ children }) {
    return (
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                  ...
                  <span style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 13, letterSpacing: "0.08em", color: "#94A3B8", textTransform: "uppercase" }}>{children}</span>span>
          </div>div>
        );
}
    ...
    if (active && payload && payload.length) {
          return (
                  <div style={{ background: "#1E2D40", border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: "10px 14px" }}>
                            <p style={{ color: COLORS.text, fontSize: 12, margin: 0, marginBottom: 6, fontWeight: 600 }}>{label}</p>p>
                    {payload.map((p) => (
...
function DashboardContent({ user, onLogout }) {
  ...
  return (
        <div style={{
                ...
                <div style={{ marginBottom: 28, display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 12 }}>
                          <div>
                                    <div style={{ fontSize: 11, letterSpacing: "0.12em", color: COLORS.muted, textTransform: "uppercase", fontWeight: 600, marginBottom: 4 }}>
                                                HubStrom - Meta Diaria
                                    </div>div>
                                    <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, letterSpacing: "-0.02em", color: "#F1F5F9" }}>
                                                Analise Comercial - <span style={{ color: COLORS.accent }}>Marco 2026</span>span>
                                    </h1>h1>
                                    <div style={{ color: COLORS.muted, fontSize: 13, marginTop: 4 }}>
                                                Periodo: 02/03 - 13/03 - 4 closers - 122 oportunidades
                                    </div>div>
                          </div>div>
                      ...
                      gap: 10,
                        ...
                        color: "#0B1120",
                                              ...
                                              <div>
                                                            <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text }}>{user.name}</div>div>
                                                            <div style={{ fontSize: 10, color: COLORS.muted, textTransform: "uppercase" }}>{user.role}</div>div>
                                              </div>div>
                                  </div>div>
                                  <button
                                                onClick={onLogout}
                      ...
                      textAlign: "right",
                                              ...
                                              <div style={{ fontSize: 11, color: COLORS.muted, marginBottom: 2 }}>Meta do Mes</div>div>
          ...
          { label: "Em Geracao", value: GERADOS, color: "#60A5FA", note: "Aguardando assinatura" },
          ...
          { label: "Desistentes", value: DESISTENTES, color: "#F87171", note: "Valor perdido" },
                              ...
                              <Card key={kpi.label} style={{ padding: "16px 18px" }}>
                                          <div style={{ fontSize: 11, color: COLORS.muted, marginBottom: 6, fontWeight: 500 }}>{kpi.label}</div>div>
                                          ...
                                          <div style={{ fontSize: 10, color: COLORS.muted, marginTop: 5 }}>{kpi.note}</div>div>
                              </Card>Card>
                            ))}
              </div>div>
          ...
          {/* Meta Progress */}
              <Card style={{ marginBottom: 22, padding: "18px 22px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                                <SectionTitle>Progresso da Meta</SectionTitle>SectionTitle>
                      ...
                      position: "relative",
                                            ...
                                            <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 2, background: "rgba(255,255,255,0.4)", borderRadius: 2 }} />
                                </div>div>
                      </div>div>
                      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, fontSize: 11, color: COLORS.muted }}>
                                <span>R$ 0</span>span>
                                <span style={{ color: "#6EE7B7" }}>R$ {TOTAL.toLocaleString("pt-BR", { minimumFractionDigits: 2 })} atingido</span>span>
                                <span>Meta: R$ {META.toLocaleString("pt-BR")}</span>span>
                      </div>div>
              </Card>Card>
          ...
          {/* Main 2-col grid */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                {/* Ranking Closers */}
                      <Card>
                                <SectionTitle>Ranking de Closers</SectionTitle>SectionTitle>
                        ...
                        <div
                                        ...
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                                                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                                                ...
                                                                fontSize: 11, fontWeight: 700, color: "#0B1120",
                                                                              ...
                                                                              <span style={{ fontWeight: 600, fontSize: 15, color: COLORS.text }}>{c.name}</span>span>
                                                          </div>div>
                                                          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 14, fontWeight: 600, color: closerColors[c.name] }}>{c.conversao}%</span>span>
                                        </div>div>
                                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6, marginBottom: 8 }}>
                                          ...
                                          { l: "Pendentes", v: c.pendente, col: "#A78BFA" },
                                                                                  ...
                                                                                  <div key={s.l} style={{ textAlign: "center" }}>
                                                                                                        <div style={{ fontSize: 16, fontWeight: 700, color: s.col }}>{s.v}</div>div>
                                                                                                        <div style={{ fontSize: 9, color: COLORS.muted, textTransform: "uppercase", letterSpacing: "0.06em" }}>{s.l}</div>div>
                                                                                    </div>div>
                                                                                ))}
                                        </div>div>
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                                          ...
                                                          <span style={{ fontSize: 11, color: COLORS.muted }}>
                                                                              Clientes conv.: <span style={{ color: "#F1F5F9" }}>{c.clientes_conv.toLocaleString("pt-BR")}</span>span>
                                                          </span>span>
                                        </div>div>
                        ...
                        <button
                                ...
                                <ResponsiveContainer width="100%" height={310}>
                                            <BarChart data={filteredTamanho} barGap={2} barCategoryGap="20%">
                                                          ...
                                                          <Legend wrapperStyle={{ paddingTop: 10, fontSize: 12 }} formatter={(v) => <span style={{ color: closerColors[v] }}>{v}</span>span>} />
                ...
                {/* Bottom Row */}
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
                      {/* Motivos de Desistencia */}
                            <Card>
                                      <SectionTitle>Motivos de Desistencia</SectionTitle>SectionTitle>
                                      <div style={{ fontSize: 11, color: COLORS.muted, marginBottom: 14 }}>Total: {motivosData.reduce((a, b) => a + b.count, 0)} desistencias</div>div>
                        ...
                        return (
                                          <div key={m.motivo}>
                                                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                                                                                <span style={{ fontSize: 12, color: COLORS.text }}>{m.motivo}</span>span>
                                                                                <span style={{ fontSize: 12, fontFamily: "'DM Mono', monospace", color: colors[i] }}>{m.count}</span>span>
                                                            </div>div>
                      ...
                      {/* Mix de Planos */}
                            <Card>
                                      <SectionTitle>Mix de Planos</SectionTitle>SectionTitle>
                          ...
                          <div key={p.name} style={{ marginBottom: 12 }}>
                                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                                                                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                                                                      ...
                                                                                      <span style={{ fontSize: 13, color: COLORS.text }}>{p.name}</span>span>
                                                                </div>div>
                                                                <span style={{ fontSize: 13, color: i === 0 ? "#F59E3F" : "#60A5FA", fontFamily: "'DM Mono', monospace" }}>{p.value}</span>span>
                                            </div>div>
                                      ...
                                      <div style={{ marginTop: 18, paddingTop: 16, borderTop: `1px solid ${COLORS.border}` }}>
                                                  <SectionTitle>Origem dos Contratos</SectionTitle>SectionTitle>
                                                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                                                    {contratoMix.map((c, i) => {
                          ...
                          return (
                                              <div key={c.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                                                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                                                                        ...
                                                                                        <span style={{ fontSize: 13, color: COLORS.text }}>{c.name}</span>span>
                                                                  </div>div>
                                                                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                                                                        ...
                                                                                        <span style={{ fontSize: 12, color: colors2[i], fontFamily: "'DM Mono', monospace", minWidth: 28, textAlign: "right" }}>{c.value}</span>span>
                                                                  </div>div>
                                              </div>div>
                                            );
        })}
                      ...
                      {/* Receita por Closer */}
                            <Card>
                                      <SectionTitle>Receita Fechada por Closer</SectionTitle>SectionTitle>
                                      <div style={{ fontSize: 11, color: COLORS.muted, marginBottom: 12 }}>Valor com desconto - Apenas contratos assinados</div>div>
                                      ...
                                      <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid ${COLORS.border}` }}>
                                                  <div style={{ fontSize: 11, color: COLORS.muted, marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600 }}>
                                                                Media de clientes/oportunidade
                                                  </div>div>
                                        {closerData.map((c) => (
                        <div key={c.name} style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                                        <span style={{ fontSize: 12, color: closerColors[c.name] }}>{c.name}</span>span>
                ...
                {/* Insights */}
                    <Card>
                            <SectionTitle>Insights e Alertas Estrategicos</SectionTitle>SectionTitle>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 12 }}>
                              {[
          { icon: "T", color: "#F59E3F", title: "Amanda lidera em conversao", text: "53.3% de taxa - a melhor da equipe. Recebe as maiores contas e tem o maior ticket medio." },
          { icon: "!", color: "#F87171", title: "Michelle com 32% de conversao", text: "9 desistencias e 9 pendentes para apenas 9 fechamentos. Foco em contabilidades menores." },
          { icon: "G", color: "#A78BFA", title: "Timing e o principal obstaculo", text: "8 de 22 desistencias (36%) sao 'Agora nao' - oportunidade para trabalhar urgencia." },
          { icon: "P", color: "#6EE7B7", title: "Isabela: maior pipeline", text: "11 pendentes no ativo - maior da equipe. Alto potencial de conversao nas proximas semanas." },
          { icon: "A", color: "#60A5FA", title: "Meta: faltam R$ 19.362", text: "72.3% da meta atingida. Com R$ 44.827 em pendentes, ha margem para superar 100%." },
          { icon: "U", color: "#FBBF24", title: "Professional x Enterprise empatados", text: "61 vs 60 oportunidades. Upsell representa apenas 7% dos contratos (8/121)." },
                    ].map((ins) => (
                                  <div
                                                  key={ins.title}
                                                  style={{
                                                ...
                                                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                                                                <span style={{ fontSize: 14, fontWeight: 700, color: ins.color, width: 22, height: 22, borderRadius: "50%", background: `${ins.color}20`, display: "flex", alignItems: "center", justifyContent: "center" }}>{ins.icon}</span>span>
                                                                <span style={{ fontSize: 13, fontWeight: 600, color: ins.color }}>{ins.title}</span>span>
                                                </div>div>
                                                <p style={{ margin: 0, fontSize: 12, color: "#94A3B8", lineHeight: 1.6 }}>{ins.text}</p>p>
                                  </div>div>
                                ))}
                            </div>div>
                    </Card>Card>
                    ...
                    <div style={{ textAlign: "center", marginTop: 20, fontSize: 11, color: "#334155" }}>
                            Dashboard HubStrom - Analise de Meta Diaria Marco 2026 - 122 oportunidades processadas
                    </div>div>
              </div>div>
          );
          }
